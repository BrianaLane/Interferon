...
#warnings.filterwarnings("ignore", category=astropy.io.fits.FITSFixedWarning)
...
class guider_observations():
    def __init__(self, guider_path, guider_as_per_pix = 0.51):
        ...
        self.guider_df = pd.DataFrame({'filename':guider_files, 'obs_datetime':np.NaN, 
                                     'exptime(s)':np.NaN})
        ...
        for g in range(len(self.guider_df)):
            ...
            time_hdr = hdr_g['UT']
            obs_dt = dt.datetime.strptime(date_hdr+'T'+time_hdr, '%Y-%m-%dT%H:%M:%S.%f')
    ...
    def inspect_guider_frame(self, guider_name, vmin=None, vmax=None):
        if isinstance(guider_name, int):
            ...
            if guider_name[-5::]=='.fits':
                ...
                print('Invalid guider filename, must be fits file or guider index integer')
                return None
        else:
            print('Need to provide guider path/filename.fits (str) or index (int) of guider frame in guider_df')
            return None
    ...
    def find_guide_stars(self, guide_ind, star_thres=10., num_bright_stars=10, star_fwhm=8.0, plot_guide_frame=False):
        hdu = fits.open(self.guider_df.iloc[guide_ind]['filename'])
        g_dat = hdu[0].data
        
        mean, median, std = sigma_clipped_stats(g_dat, sigma=3.0) 
        daofind = DAOStarFinder(fwhm=star_fwhm, threshold=star_thres*std, peakmax=64000, 
                                exclude_border=True, brightest=num_bright_stars) 
        ...
        if plot_guide_frame:
            ...
            for i in range(len(sources_df)):
                aperture = CircularAperture((sources_df.iloc[i]['xcentroid'], sources_df.iloc[i]['ycentroid']), r=star_fwhm)
    ...
    def measure_guide_star_params(self, guide_ind, sources_df, plot_star_cutouts=False):
        hdu = fits.open(self.guider_df.iloc[guide_ind]['filename'])
        g_dat = hdu[0].data
        ...
        for s in range(len(sources_df)):
            ...
            if (xp > (off-1)) and (yp > (off-1)):
                ...
                g = models.Gaussian2D(amplitude=so['peak']-mean, x_mean=x_cen, y_mean=y_cen, x_stddev=1.0, y_stddev=1.0)
                ...
                if plot_star_cutouts:
                    ...
                    plt.text(1, 2, 'fwhm:'+str(np.round(fwhm_fit,2)), fontsize=12, color='white')
                    plt.text(1, 4, 'peak:'+str(np.round(fit_g.amplitude.value,2)), fontsize=12, color='white')
                    plt.text(1, 6, 'mag:'+str(np.round(mag_fit,2)), fontsize=12, color='white')
                    plt.title(s)
                    plt.show()
    ...
    def find_ref_guide_frame(self, guide_ind_list, star_thres=10., num_bright_stars=10, star_fwhm=8.0, 
                             fwhm_lim=(0.5,10), mag_lim=10):
            
        for g in range(len(guide_ind_list)):
            guide_ind = guide_ind_list[g]
            guide_file = self.guider_df.iloc[guide_ind]['filename']

            source_ex = self.find_guide_stars(guide_ind, star_thres=star_thres, 
                                              num_bright_stars=num_bright_stars, star_fwhm=star_fwhm)
            if len(source_ex)>1:
                source_fit = self.measure_guide_star_params(guide_ind, source_ex)
